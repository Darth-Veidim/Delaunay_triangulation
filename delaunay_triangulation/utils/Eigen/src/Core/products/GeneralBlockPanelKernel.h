...
void evaluateProductBlockingSizesHeuristic(Index& k, Index& m, Index& n, Index num_threads = 1)
{
  ...
  if (num_threads > 1) {
    ...
    //  find a reliable way to get the actual amount of cache per core to use for 2nd level blocking, that is:
    ...
    else if(old_k==k)
    {
      ...
      // : part of this blocking strategy is now implemented within the kernel itself, so the L1-based heuristic here should be obsolete.
...
void loadQuadublePacket(const Scalar* b, DoublePacket<RealPacket>& dest,
...
public:
  ...
  enum {
    ...
    // : should depend on NumberOfRegisters
    nr = 4,
    mr = ResPacketSize,
...
//     const Index depth2     = depth & ~1;
    ...
    if(mr>=3*Traits::LhsProgress)
    {
      ...
      const Index l1 = defaultL1CacheSize; // in Bytes, , l1 should be passed to this function.
...
//       Index k=0;
//       if(PacketSize==8) //  enable vectorized transposition for PacketSize==4
//       {
...
//   }
  ...
  if(nr>=4)
  {
    for(Index j2=packet_cols8; j2<packet_cols4; j2+=4)
    {
      ...
      Index k=0;
      if((PacketSize%4)==0) //  enable vectorized transposition for PacketSize==2 ??
      {
        for(; k<peeled_k; k+=PacketSize) {
