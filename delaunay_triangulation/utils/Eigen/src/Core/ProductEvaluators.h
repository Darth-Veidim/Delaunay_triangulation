...
//  we should apply that rule only if that's really helpful
...
//  shall we handle nested_eval here?,
...
// : we could enable them for different scalar types when the product is not vectorized.
...
//  we should apply that rule if that's really helpful
...
//  we could probably enable these rules for any product, i.e., not only Dense and DefaultProduct
...
void EIGEN_DEVICE_FUNC outer_product_selector_run(Dst& dst, const Lhs &lhs, const Rhs &rhs, const Func& func, const false_type&)
{
  ...
  //  if cols is large enough, then it might be useful to make sure that lhs is sequentially stored
  //  not very good if rhs is real and lhs complex while alpha is real too
...
void EIGEN_DEVICE_FUNC outer_product_selector_run(Dst& dst, const Lhs &lhs, const Rhs &rhs, const Func& func, const true_type&)
{
  ...
  //  if rows is large enough, then it might be useful to make sure that rhs is sequentially stored
  //  not very good if lhs is real and rhs complex while alpha is real too
...
struct generic_product_impl<Lhs,Rhs,DenseShape,DenseShape,OuterProduct>
{
  ...
  //  it would be nice to be able to exploit our *_assign_op functors for that purpose
...
struct generic_product_impl<Lhs,Rhs,DenseShape,DenseShape,CoeffBasedProductMode> 
{
  ...
  void eval_dynamic(Dst& dst, const Lhs& lhs, const Rhs& rhs, const Func &func)
  {
    ...
    // : in c++11 this should be auto, and extractScalarFactor should also return auto
    ...
    : evaluator_base<Product<Lhs, Rhs, LazyProduct> >
{
  ...
  explicit product_evaluator(const XprType& xpr)
    : m_lhs(xpr.lhs()),
      m_rhs(xpr.rhs()),
      m_lhsImpl(m_lhs),     //  the creation of the evaluator objects should result in a no-op, but check that!
  ...
  enum {
    ...
    Flags = ((unsigned int)(LhsFlags | RhsFlags) & HereditaryBits & ~RowMajorBit)
          ...
          //  enable vectorization for mixed types
   ...
   * : this seems possible when the result is a vector
   */
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
...
protected:
  ...
  // : Get rid of m_innerDim if known at compile time
  Index m_innerDim;
};
...
public:
  enum {
    ...
    //  currently we need same types, but in the future the next rule should be the one
...
protected:
  ...
  EIGEN_STRONG_INLINE PacketType packet_impl(Index row, Index col, Index id, internal::false_type) const
  {
    enum {
      ...
      DiagonalPacketLoadMode = EIGEN_PLAIN_ENUM_MIN(LoadMode,((InnerSize%16) == 0) ? int(Aligned16) : int(evaluator<DiagonalType>::Alignment)) //  hardcoded 16!!
    };
    return internal::pmul(m_matImpl.template packet<LoadMode,PacketType>(row, col),
  ...
  : diagonal_product_evaluator_base<Rhs, typename Lhs::DiagonalVectorType, Product<Lhs, Rhs, LazyProduct>, OnTheLeft>
{
  ...
  {
    // : NVCC used to complain about the template keyword, but we have to check whether this is still the case.
...
struct permutation_matrix_product<ExpressionType, Side, Transposed, DenseShape>
{
    ...
    static inline void run(Dest& dst, const PermutationType& perm, const ExpressionType& xpr)
    {
      ...
      //  we need an is_same for expression that is not sensitive to constness. For instance
...
//  could we unify Transpositions and Permutation into a single "shape"??
