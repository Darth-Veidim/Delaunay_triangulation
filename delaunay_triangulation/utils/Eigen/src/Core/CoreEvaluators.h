...
{
  //  that's not very nice to have to propagate all these traits. They are currently only needed to handle outer,inner indices.
  ...
  : evaluator_base<CwiseUnaryView<UnaryOp, ArgType> >
{
  ...
  enum {
    ...
    Alignment = 0 //  it is not very clear why alignment is necessarily lost...
  };
...
//  perhaps the PlainObjectType could be provided by Derived::PlainObject ?
  ...
  : public mapbase_evaluator<Map<PlainObjectType, MapOptions, StrideType>, PlainObjectType>
{
  ...
  typedef typename XprType::Scalar Scalar;
  // : should check for smaller packet types once we can handle multi-sized packet types
...
// : This evaluator does not actually use the child evaluator;
                      ...
                      typename Block<ArgType, BlockRows, BlockCols, InnerPanel>::PlainObject>
{
  ...
  {
    // : for the 3.3 release, this should be turned to an internal assertion, but let's keep it as is for the beta lifetime
...
//  enable vectorization for Select
  ...
  : evaluator_base<Reverse<ArgType, Direction> >
{
  ...
  enum {
    ...
    //  enable DirectAccess with negative strides?
    ...
    Alignment = 0 //  in some rare cases, Alignment could be preserved, like a Vector4f.
  };
  ...
  void writePacket(Index row, Index col, const PacketType& x)
  {
    //  we could factorize some code with packet(i,j)
    enum {
      PacketSize = unpacket_traits<PacketType>::size,
      OffsetRow  = ReverseRow && IsColMajor ? PacketSize : 1,
      OffsetCol  = ReverseCol && IsRowMajor ? PacketSize : 1
