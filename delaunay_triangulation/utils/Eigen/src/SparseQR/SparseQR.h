...
void SparseQR<MatrixType,OrderingType>::analyzePattern(const MatrixType& mat)
{
  ...
  m_R.reserve(2*mat.nonZeros()); // Get a more accurate estimation through symbolic factorization with the etree
...
void SparseQR<MatrixType,OrderingType>::factorize(const MatrixType& mat)
{
  ...
  for (StorageIndex col = 0; col < n; ++col)
  {
    ...
    for (Index i = nzcolR-1; i >= 0; i--)
    {
      ...
      // : tval -= tdot * m_Q.col(curIdx) should amount to the same (need to check/add support for efficient "dense ?= sparse")
    ...
    if(nonzeroCol < diagSize)
    {
      ...
      //  this step should call the Householder module.
...
struct SparseQR_QProduct : ReturnByValue<SparseQR_QProduct<SparseQRType, Derived> >
{
  ...
  bool m_transpose; //  this actually means adjoint
};
...
{  
  ...
  // To use for operations with the transpose of Q  this is the same as adjoint at the moment
...
//  this actually represents the adjoint of Q
template<typename SparseQRType>
