  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex> >
{
  ...
  public:
    ...
    {
      //  optimize the uncompressed mode to avoid moving and allocating the data twice
      makeCompressed();
...
protected:
    ...
    /** \internal assign \a diagXpr to the diagonal of \c *this
      ...
      * 
      * : some piece of code could be isolated and reused for a general in-place update strategy.
      * : if we start to defer the insertion of some elements (i.e., case 2.c executed once),
...
{
  eigen_assert(!isCompressed());
  // , in practice we should be able to use m_innerNonZeros for that task
...
EIGEN_DONT_INLINE SparseMatrix<Scalar,_Options,_StorageIndex>& SparseMatrix<Scalar,_Options,_StorageIndex>::operator=(const SparseMatrixBase<OtherDerived>& other)
{
  ...
  if (needToTranspose)
  {
    ...
    // pass 1
    //  the above copy could be merged with that pass
...
EIGEN_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_StorageIndex>::Scalar& SparseMatrix<_Scalar,_Options,_StorageIndex>::insertCompressed(Index row, Index col)
{
  ...
  //  let's make sure sizeof(long int) == sizeof(std::size_t)
